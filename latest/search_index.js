...

{
    ...
    "category": "section",
    "text": "A differential equation in the form: \\begin{equation} 	\\frac{\\dd x}{\\dd t} = f(x)\\com \\end{equation} can also be written in an integral form: \\begin{equation} 	x(t) = \\int_{t_0}^{t} f(x(s))\\,\\dd s\\per \\end{equation} In a similar manner, a stochastic differential equation \\begin{equation} 	\\dd x = f(x)\\,\\dd t + g(x)\\,\\dd W_t\\com \\end{equation} with dd W_t a white-noise process, can be written in an integral form as: \\begin{equation} 	x(t) = \\int_{t_0}^{t} f(x(s))\\,\\dd s + \\int_{t_0}^{t} g(x(s))\\,\\dd W_s \\per \\end{equation} Of course now, the last integral is a stochastic integral and there is not a single straight-forward way of computing it –- there are a lot of different ways we can approximate it as a Riemannian sum and each of them leads to a different answer. The two most popular ways for computing such stochastic integrals are:colorgreentextIt int_t_0^t g(x(s))dd W_sapproxsum_j gleft(x(t_j)right)(W_j+1-W_j)com\ncolorBluetextStranotovich int_t_0^t g(x(s))dd W_s approx sum_j gleft(xleft(half(t_j+t_j+1)right)right)(W_j+1-W_j)perAlthough that the two definitions would converge to the very same result if the x(t) was continuous, in SDE\'s that is not the case; the two different definitions give thoroughly different results. And to overcome that they come along with different chain rules, i.e. chain rules that are not necessarily the same as those in plain old calculus.An SDE can be written also in differential form. Because we cannot formally form dd Wdd t, since W is nowhere differentiable, we write an SDE in differential form as:colorgreentextIt dd x_t = f(x_t)dd t + g(x_t)dd W_tcom\ncolorBluetextStranotovich dd x_t = f(x_t)dd t + g(x_t)circdd W_tperThe circle in g(x_t)circdd W_t is used to differentiate between Itô or Stratonovich calculus.A variable change y=G(x) is done as follows according to the two different calculi:colorgreentextIt dd y_t = fracdd Gdd xdd x_t + half g(x_t)^2 fracdd^2 Gdd x^2dd t =left fracdd Gdd xf(x_t) + half g(x_t)^2 fracdd^2 Gdd x^2rightdd t + fracdd Gdd xg(x_t)dd W_tcom\ncolorBluetextStranotovich dd y_t  = fracdd Gdd xdd x_t =fracdd Gdd x f(x_t) dd t + fracdd Gdd xg(x_t)dd W_tperThe above are the so called stochastic chain rules. All derivatives of G are evaluated at x_t.It\'s easy to see that the extra drift-term in Itô\'s interpretation of the stochastic integral, i.e., colorgreenhalf g^2 dd^2Gdd x^2  is exactly equal to the ensemble mean of the Stratonovich stochastic integral. This is that case because, by construction, the Itô stochastic integral has zero ensemble mean since at every instant the noise is multiplied with g evaluated before the action of the noise occurs; g and dd W are uncorrelated and thus: \\begin{equation} {\\color{green}\\laa g(x_t)\\dd W_t \\raa =0}\\quad\\text{while}\\quad {\\color{Blue}\\laa g(x_t)\\circ\\dd W_t \\raa \\ne 0}\\per \\end{equation} The above is demonstrated by evaluating the simple stochastic integral:colorgreentextIt laa int_t_0^t W_sdd W_s raa approxsum_j laa W_j(W_j+1-W_j)raa\ncolorgreenhspace73em = sum_j laa W_j W_j+1raa - laa W_jW_jraa sim sum_j t_j - t_j = 0 com\ncolorBluetextStranotovich laaint_t_0^t W_scircdd W_sraa approx sum_j laa frac12(W_j + W_j+1) (W_j+1-W_j)raa \ncolorBluehspace73em = frac12sum_j laa W_j+1 W_j+1raa - laa W_j W_jraa  sim frac12sum_j t_j+1 - t_j = fract2perSDEs rarely can be solved in closed form; most often numerical solution of SDEs is brought to the rescue. Itô calculus has the advantage that is very easily implemented numerically. On the other hand, Stratonovich calculus coincides with that from normal calculus and this stems from the fact that it vies the white noise process as a series of colored noise processes with the de-correlation time tending to zero. This last fact is what made Stratonovich calculus more popular in the physics community. A nice discussion on the differences and similarities between the two calculi is done by van Kampen."
},
